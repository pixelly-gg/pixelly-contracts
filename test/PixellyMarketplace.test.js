...
contract(
  "Core ERC721 tests for PixellyNFT",
  function ([owner, minter, buyer, feeRecipient]) {
    ...
    beforeEach(async function () {
      this.nft = await PixellyNFT.new({ from: owner });
      this.marketplace = await PixellyMarketplace.new(
    ...
    describe("Buying Item", function () {
      ...
      it("reverts when seller doesnt own the item", async function () {
        ...
        await expectRevert(
          this.marketplace.buyItem(this.nft.address, firstTokenId, {
            from: buyer,
            value: pricePerItem,
          }),
      ...
      it("reverts when buying before the scheduled time", async function () {
        ...
        await expectRevert(
          this.marketplace.buyItem(this.nft.address, secondTokenId, {
            from: buyer,
            value: pricePerItem,
          }),
