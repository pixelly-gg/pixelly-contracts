...
const PixellyAuction = artifacts.require("PixellyAuctionMock");
const PixellyAuctionReal = artifacts.require("PixellyAuction");
const BiddingContractMock = artifacts.require("BiddingContractMock");
const MockERC20 = artifacts.require("MockERC20");

contract("PixellyAuction", (accounts) => {
  const [
    admin,
    smartContract,
    platformFeeAddress,
  ...
  beforeEach(async () => {
    this.token = await PixellyNFT.new({ from: admin });
    ...
    this.auction = await PixellyAuction.new(platformFeeAddress, { from: admin });
  ...
  describe("Contract deployment", () => {
    it("Reverts when platform fee recipient is zero", async () => {
      await expectRevert(
        PixellyAuction.new(constants.ZERO_ADDRESS, { from: admin }),
    ...
    describe("validation", async () => {
      ...
      it("fails if endTime is in the past", async () => {
        ...
        await expectRevert(
          ...
          "PixellyAuction.createAuction: End time passed. Nobody can bid."
        );
      });
      ...
      it("fails if endTime greater than startTime", async () => {
        ...
        await expectRevert(
          ...
          "PixellyAuction.createAuction: End time must be greater than start"
        );
      });
      ...
      it("fails if token already has auction in play", async () => {
        ...
        await expectRevert(
          ...
          "PixellyAuction.createAuction: Cannot relist"
        );
      });
      ...
      it("fails if you dont own the token", async () => {
        ...
        await expectRevert(
          ...
          "PixellyAuction.createAuction: Not owner and or contract not approved"
        );
      });
      ...
      it("fails if token does not exist", async () => {
        ...
        await expectRevert(
          this.auction.createAuction(this.token.address, "99", "1", "1", "11", {
            from: minter,
          }),
  ...
  describe("placeBid()", async () => {
    describe("validation", () => {
      ...
      it("will revert if sender is smart contract", async () => {
        this.biddingContract = await BiddingContractMock.new(
          this.auction.address
        );
        await expectRevert(
          this.biddingContract.bid(this.token.address, TOKEN_ONE_ID, {
            from: bidder,
            value: ether("0.2"),
          }),
          "PixellyAuction.placeBid: No contracts permitted"
        );
      });

      it("will fail with 721 token not on auction", async () => {
        await expectRevert(
          this.auction.placeBid(this.token.address, 999, {
            from: bidder,
            value: 1,
          }),
          "PixellyAuction.placeBid: Bidding outside of the auction window"
        );
      });

      it("will fail with valid token but no auction", async () => {
        await expectRevert(
          this.auction.placeBid(this.token.address, TOKEN_TWO_ID, {
            from: bidder,
            value: 1,
          }),
          "PixellyAuction.placeBid: Bidding outside of the auction window"
      ...
      it("will fail when auction finished", async () => {
        ...
        await expectRevert(
          this.auction.placeBid(this.token.address, TOKEN_ONE_ID, {
            from: bidder,
            value: 1,
          }),
          "PixellyAuction.placeBid: Bidding outside of the auction window"
      ...
      it("will fail when outbidding someone by less than the increment", async () => {
        await this.auction.setNowOverride("2");
        await this.auction.placeBid(this.token.address, TOKEN_ONE_ID, {
          from: bidder,
          value: ether("0.2"),
        });
        ...
        await expectRevert(
          ...
          "PixellyAuction.placeBid: Failed to outbid highest bidder"
        );
      });
    });
    ...
    describe("successfully places bid", () => {
      ...
      it("places bid and you are the top owner", async () => {
        ...
        const { _bidder, _bid } = await this.auction.getHighestBidder(
          this.token.address,
          TOKEN_ONE_ID
        );
      ...
      it("will refund the top bidder if found", async () => {
        ...
        const { _bidder: originalBidder, _bid: originalBid } =
          await this.auction.getHighestBidder(this.token.address, TOKEN_ONE_ID);
      ...
      it("successfully increases bid", async () => {
        ...
        const bidderTracker = await balance.tracker(bidder);
        const receipt = await this.auction.placeBid(
          this.token.address,
          TOKEN_ONE_ID,
          { from: bidder, value: ether("0.2") }
        );

        expect(await bidderTracker.delta()).to.be.bignumber.equal(
          ether("0.2")
            .add(await getGasCosts(receipt))
            .mul(new BN("-1"))
        );
        ...
        const receipt2 = await this.auction.placeBid(
          this.token.address,
          TOKEN_ONE_ID,
          { from: bidder, value: ether("1") }
        );
        ...
        expect(await bidderTracker.delta()).to.be.bignumber.equal(
          ether("1")
            .sub(ether("0.2"))
            .add(await getGasCosts(receipt2))
            .mul(new BN("-1"))
        );
      ...
      it("successfully outbid bidder", async () => {
        ...
        const receipt2 = await this.auction.placeBid(
          this.token.address,
          TOKEN_ONE_ID,
          { from: bidder2, value: ether("1") }
        );

        // check that the bidder has only really spent 0.8 ETH plus gas due to 0.2 ETH refund
        expect(await bidder2Tracker.delta()).to.be.bignumber.equal(
          ether("1")
            .add(await getGasCosts(receipt2))
            .mul(new BN("-1"))
        );
  ...
  describe("withdrawBid()", async () => {
    ...
    it("fails with withdrawing a bid which does not exist", async () => {
      await expectRevert(
        ...
        "PixellyAuction.withdrawBid: You are not the highest bidder"
      );
    });
    ...
    it("fails with withdrawing when lockout time not passed", async () => {
      ...
      await expectRevert(
        ...
        "PixellyAuction.withdrawBid: Cannot withdraw until lock time has passed"
      );
    });
    ...
    it("fails when withdrawing after auction end", async () => {
      ...
      await expectRevert(
        ...
        "PixellyAuction.withdrawBid: Past auction end"
      );
    });
    ...
    it("successfully withdraw the bid", async () => {
      ...
      const receipt = await this.auction.withdrawBid(
        this.token.address,
        TOKEN_ONE_ID,
        { from: bidder }
      );
    ...
    describe("validation", () => {
      ...
      it("cannot result if not an owner", async () => {
        await expectRevert(
          ...
          "PixellyAuction.resultAuction: Sender must be item owner"
        );
      });
      ...
      it("cannot result if auction has not ended", async () => {
        await expectRevert(
          ...
          "PixellyAuction.resultAuction: The auction has not ended"
        );
      });
      ...
      it("cannot result if the auction is reserve not reached", async () => {
        ...
        await expectRevert(
          ...
          "PixellyAuction.resultAuction: reserve not reached"
        );
      });

      it("cannot result if the auction has no winner", async () => {
        // Lower reserve to zero
        await this.auction.updateAuctionReservePrice(
          this.token.address,
          TOKEN_ONE_ID,
          "0",
          { from: minter }
        ...
        await expectRevert(
          ...
          "PixellyAuction.resultAuction: no open bids"
        );
      });
      ...
      it("cannot result if the auction if its already resulted", async () => {
        ...
        // result it
        await this.auction.resultAuction(this.token.address, TOKEN_ONE_ID, {
          from: minter,
        });
    ...
    describe("successfully resulting an auction", async () => {
      ...
      it("transfer funds to the token creator and platform", async () => {
        ...
        expect(platformChanges).to.be.bignumber.equal(
          ether("0.4")
            .sub(ether("0.1")) // total minus reserve
            .div(new BN("1000"))
    ...
    describe("validation", async () => {
      it("cannot cancel if not an admin", async () => {
        await expectRevert(
          ...
          "PixellyAuction.cancelAuction: Sender must be item owner"
        );
      });
      ...
      it("cannot cancel if auction already cancelled", async () => {
        ...
        await this.auction.cancelAuction(this.token.address, TOKEN_ONE_ID, {
          from: minter,
        });
  ...
  describe("create, cancel and re-create an auction", async () => {
    ...
    it("once created and then cancelled, can be created and resulted properly", async () => {
      ...
      // Result it
      const { receipt } = await this.auction.resultAuction(
        this.token.address,
        TOKEN_ONE_ID,
        { from: minter }
      );
  ...
  describe("reclaimERC20()", async () => {
    describe("validation", async () => {
      ...
      it("can reclaim Erc20", async () => {
        ...
        expect(
          await this.mockToken.balanceOf(this.auction.address)
        ).to.be.bignumber.equal(TWENTY_TOKENS);
        ...
        expect(
          await this.mockToken.balanceOf(admin)
        ).to.be.bignumber.greaterThan(adminBalanceBeforeReclaim);
      });
    });
  });
