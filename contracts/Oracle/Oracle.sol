...
contract Oracle is Ownable {
    ...
    function getAveragePrice(address token) public view returns (uint256 price) {
        ...
        if (token == anchorToken) {
            price = amount;
        } else if (UniswapV2Library.pairFor(factory, token, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    UniswapV2Library.pairFor(factory, token, intermediate) != address(0) &&
                    UniswapV2Library.pairFor(factory, intermediate, anchorToken) != address(0)
                ) {
    ...
    function getCurrentPrice(address token) public view returns (uint256 price) {
        ...
        if (token == anchorToken) {
            ...
            (uint256 reserve0, uint256 reserve1) = UniswapV2Library.getReserves(factory, token, anchorToken);
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    ...
                    (uint256 reserve0, uint256 reserve1) = UniswapV2Library.getReserves(factory, token, intermediate);
                    uint256 amountOut = 10**tokenDecimal.mul(reserve1).div(reserve0);
                    (uint256 reserve2, uint256 reserve3) = UniswapV2Library.getReserves(factory, intermediate, anchorToken);
